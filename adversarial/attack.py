...
class Attack(object):
    ...
    def set_attack_mode(self, mode, target_map_function=None):
        ...
        if mode == "default":
            ...
            raise ValueError(
                mode + " is not a valid mode. [Options: default, targeted, least_likely]")
    ...
    def save(self, data_loader, save_path=None, verbose=True):
        ...
        if (self._attack_mode is 'targeted') and (self._target_map_function is None):
            raise ValueError(
                "save is not supported for target_map_function=None")
        ...
        for step, (images, labels) in enumerate(data_loader):
            ...
            if verbose:
                with torch.no_grad():
                    ...
                    delta = (adv_images - images.to(self.device)
                             ).view(batch_size, -1)
