    ...
    def train_and_evaluate(self):
        ...
        for dataset_name in ds:
            ...
            if self.args.eval_batch_f1:
                allx = np.concatenate(
                    (ds['train']['X'], ds['val']['X'], ds['test']['X']))
                ally = np.concatenate(
                    (ds['train']['y'], ds['val']['y'], ds['test']['y']))
            ...
            for loss in _losses:
                model, optimizer, training_loss = self.make(
                    name="loss_{}".format(loss), input_shape=input_shape, loss=loss)
                ...
                model_tb_path = self.experiment.tensorboard_log_path.joinpath(
                    '{}_{}'.format(dataset_name, loss))
                tensorboard_callback = keras.callbacks.TensorBoard(
                    log_dir=model_tb_path)
                ...
                self.experiment.log.info(
                    "Fitting: {} on {}".format(loss, dataset_name))
                ...
                if not self.args.eval_batch_f1:
                    ...
                    print(
                        f"Going through the dataset {self.args.epochs} times.\n"+("="*30))
                    for epoch in range(self.args.epochs):
                        if early_stopping:
                            print(
                                "Early Stopping at Epoch {}/{}".format(epoch, self.args.epochs))
                            break
                        ...
                        for x, y in train_set:
                            ...
                            gradients = tape.gradient(
                                loss_val, model.trainable_variables)
                            ...
                            if self.args.eval_batch_f1:
                                ...
                                threshold = 0.5
                                # train set
                                train_predictions = tf.where(
                                    model.predict(x) > threshold, 1, 0)
                                train_f1 = sklearn.metrics.f1_score(
                                    y, train_predictions)

                                # all data
                                all_predictions = tf.where(
                                    model.predict(allx) > threshold, 1, 0)
                                all_f1 = sklearn.metrics.f1_score(
                                    ally, all_predictions)
                        ...
                        for split in ['train', 'val', 'test']:
                            evaluation = model.evaluate(
                                ds[split]['X'], ds[split]['y'], batch_size=self.args.batch_size, verbose=0)
                            with summary_writer[split].as_default():
                                tf.summary.scalar(
                                    'loss', epoch_best_loss, step=epoch)
                                tf.summary.scalar(
                                    'patience', epoch_best_loss, step=epoch)
                                for name, value in zip(model.metrics_names, evaluation):
                                    tf.summary.scalar(name, value, step=epoch)

                        print("(Dataset: {}, Loss: {}) Epoch [{}]: val_loss: {}".format(
                            dataset_name, loss, epoch, epoch_best_loss))

                    if self.args.eval_batch_f1:
                        # Write the data in a csv:
                        f1_storage_path = str(self.experiment.path.joinpath(
                            "f1_per_batch_loss-{}_dataset-{}_batch_size-{}.csv".format(loss, dataset_name, self.args.batch_size)))
                ...
                for split in ['train', 'test']:
                    self.experiment.log.info(
                        "{} on {}:{} evaluation".format(loss, dataset_name, split))
                    self.experiment.predictions[dataset_name][loss][split] = model.predict(
                        ds[split]['X'], batch_size=self.args.batch_size)
                ...
                model_path = str(self.experiment.path.joinpath(
                    "model_{}_{}.h5".format(dataset_name, loss)))
                timing_path = str(self.experiment.path.joinpath(
                    "model_{}_{}_timing.pkl".format(dataset_name, loss)))
