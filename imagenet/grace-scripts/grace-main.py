...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # define loss function (criterion) and optimizer - using Adam for early stopping. 
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        print("Valid - Loss: {:.6f} | Accuracy: {:.6f}".format(valid_loss, acc1))
        ...
        if lowest_valid_loss is None or valid_loss < lowest_valid_loss: 
            ...
            fname = "/".join(["run-two-models", str(args.run_name) + "-" + "model_best.pth.tar"])
        ...
        checkpoint_fname = "/".join(["deep1-early-stopping",
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        print("batch time:{:.5f}, loss: {:.5f}".format(time.time() - end, loss.item()))
        end = time.time()
...
def save_checkpoint(state, is_best, filename='ce_checkpoint.pth.tar'):
    torch.save(state, filename)
    if is_best:
        shutil.copyfile(filename, 'ce_model_best.pth.tar')
...
if __name__ == '__main__':
    ...
    # python3 grace-main.py --gpu 0 --run_name es-baseline /app/timeseries/data/imagenet-data
