        ...
        for x in range(len(models_list)):
            ...
            for tau in test_thresholds:
                ...
                for i, (inputs, labels) in enumerate(test_loader):
                    ...
                    output = model(inputs)
                    labels = torch.zeros(len(labels), 4).to(device).scatter_(1, labels.unsqueeze(1), 1.).to(device)
...
if __name__ == '__main__':
    ...
    for run_number in range(num_runs): 
        ...
        get_metrics(device="cuda:3", batch_size=512, seed=11,
                    results_path="/app/timeseries/multiclass_src/results/wine",
                    models_path="/app/timeseries/multiclass_src/models/wine",
